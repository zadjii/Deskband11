...
public partial class ExtensionService : IExtensionService, IDisposable
{
    ...
    private const string RegistrationProperty = "Registration";
    private const string RegistrationValue = "#text";
    ...
    private void InstallPackageUnderLock(Package package)
    {
        ...
        {
            return IsValidDeskbandExtension(package);
        }).Result;
        ...
        if (isExtension && extension is not null)
        {
            ...
            Task.Run(async () =>
            {
                ...
                try
                {
                    List<ExtensionWrapper> wrappers = await CreateDeskbandWrappersForExtension(extension);
    ...
    private static async Task<IsExtensionResult> IsValidDeskbandExtension(Package package)
    {
        IReadOnlyList<AppExtension> extensions = await AppExtensionCatalog.Open("com.zadjii.deskband").FindAllAsync();
        foreach (AppExtension? extension in extensions)
        {
            if (package.Id?.FullName == extension.Package?.Id?.FullName)
            {
                (IPropertySet deskbandProvider, List<string> paths) = await GetDeskbandExtensionPropertiesAsync(extension);

                return new(deskbandProvider is not null && paths.Count != 0, extension);
            }
        }
    ...
    private static async Task<(IPropertySet? DeskbandProvider, List<string> Paths)> GetDeskbandExtensionPropertiesAsync(AppExtension extension)
    {
        ...
        IPropertySet? deskbandProvider = GetSubPropertySet(properties, "DeskbandProvider");
        if (deskbandProvider is null)
        {
            return (null, paths);
        }

        //IPropertySet? activation = GetSubPropertySet(deskbandProvider, "Registration");
        //if (activation is null)
        //{
        //    return (deskbandProvider, paths);
        //}

        // Handle case where extension creates multiple instances.
        paths.AddRange(GetRegistrationFileList(deskbandProvider));

        return (deskbandProvider, paths);
    }

    private static async Task<IEnumerable<AppExtension>> GetInstalledAppExtensionsAsync() => await AppExtensionCatalog.Open("com.zadjii.deskband").FindAllAsync();
    ...
    private static async Task<List<ExtensionWrapper>> CreateDeskbandWrappersForExtension(AppExtension extension)
    {
        (IPropertySet? deskbandProvider, List<string> paths) = await GetDeskbandExtensionPropertiesAsync(extension);

        if (deskbandProvider is null || paths.Count == 0)
        {
            return [];
        }
        ...
        {
            ExtensionWrapper extensionWrapper = CreateDeskbandExtensionWrapper(extension, deskbandProvider, path);
            wrappers.Add(extensionWrapper);
        }
    ...
    private static ExtensionWrapper CreateDeskbandExtensionWrapper(AppExtension extension, IPropertySet deskbandProvider, string relativePath)
    {
        ExtensionWrapper extensionWrapper = new(extension, relativePath);
    ...
    private static List<string> GetRegistrationFileList(IPropertySet deskbandPropSet)
    {
        List<string> fileList = new();

        IPropertySet? registrationPropSet = deskbandPropSet[RegistrationProperty] as IPropertySet;
        if (registrationPropSet is null)
        {
            return fileList;
        }

        //foreach (KeyValuePair<string, object> kv in registrationPropSet)
        //{
        //    Debug.WriteLine($"{kv.Key}: {kv.Value}");
        //}
        string? singleFile = GetProperty(registrationPropSet, RegistrationValue);
        ...
        {
            object[]? fileArray = registrationPropSet[RegistrationProperty] as object[];
    ...
    public Task<IEnumerable<IExtensionWrapper>> GetInstalledExtensionsAsync(ProviderType providerType, bool includeDisabledExtensions = false)
    {
