...
internal sealed class WindowProcess
{
    ...
    internal bool IsShellProcess
    {
        get
        {
            nint hShellWindow = NativeMethods.GetShellWindow();
    ...
    {
        nint processHandle = NativeMethods.OpenProcess(ProcessAccessFlags.QueryLimitedInformation, true, (int)pid);
        StringBuilder processName = new(MaximumFileNameLength);
        ...
        if (NativeMethods.GetProcessImageFileName(processHandle, processName, MaximumFileNameLength) != 0)
        {
            ...
            return processName.ToString().Split('\\').LastOrDefault(string.Empty);// .Reverse().ToArray()[0];
        }
        else
        {
            _ = Win32Helpers.CloseHandleIfNotNull(processHandle);
    ...
    {
        nint processHandle = NativeMethods.OpenProcess(ProcessAccessFlags.AllAccess, true, (int)pid);
