...
public sealed class Window
{
    ...
    internal string Title
    {
        get
        {
            ...
            if (sizeOfTitle++ > 0)
            {
                StringBuilder titleBuffer = new(sizeOfTitle);
                int numCharactersWritten = NativeMethods.GetWindowText(hwnd, titleBuffer, sizeOfTitle);
                return numCharactersWritten == 0 ? string.Empty : titleBuffer.ToString();
            }
            else
            {
                return string.Empty;
    ...
    {
        StringBuilder windowClassName = new(300);
        int numCharactersWritten = NativeMethods.GetClassName(hwnd, windowClassName, windowClassName.MaxCapacity);
    ...
    private static WindowProcess CreateWindowProcessInstance(IntPtr hWindow)
    {
        lock (_handlesToProcessCache)
        {
            ...
            if (!_handlesToProcessCache.ContainsKey(hWindow))
            {
                ...
                uint processId = WindowProcess.GetProcessIDFromWindowHandle(hWindow);
                uint threadId = WindowProcess.GetThreadIDFromWindowHandle(hWindow);
                string processName = WindowProcess.GetProcessNameFromProcessID(processId);
            ...
            if (string.Equals(_handlesToProcessCache[hWindow].Name, "ApplicationFrameHost.exe", StringComparison.OrdinalIgnoreCase))
            {
                new Task(() =>
                {
                    EnumWindowsProc callbackptr = new((IntPtr hwnd, IntPtr lParam) =>
                    {
                        ...
                        {
                            uint childProcessId = WindowProcess.GetProcessIDFromWindowHandle(hwnd);
                            uint childThreadId = WindowProcess.GetThreadIDFromWindowHandle(hwnd);
                            string childProcessName = WindowProcess.GetProcessNameFromProcessID(childProcessId);
